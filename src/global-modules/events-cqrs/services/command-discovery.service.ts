...
export class CommandDiscoveryService {
  ...
  async discoverCommands(): Promise<Array<{ commandClass: any; handlerClass: any }>> {
    ...
    for (const provider of providers) {
      if (provider.instance) {
        ...
        const commandClass = Reflect.getMetadata('__commandHandler__', handlerClass);
        ...
        if (commandClass) {
          ...
          this.logger.log(`Discovered command: ${commandClass.name} -> ${handlerClass.name}`);
        }
      }
    }
